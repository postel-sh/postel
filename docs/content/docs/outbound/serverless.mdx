...
Start conservative — a `deadline` well under your platform's hard timeout, and a `maxMessages` sized to your typical backlog between cron runs — then watch `result.reachedDeadline` and `postel.health().outboxDepth` (see [Observability](/docs/outbound/observability)) to see whether you're keeping up.
