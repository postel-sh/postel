...
> **Status: shipping in v0.2.0.** The outbound runtime is implemented and runs today against the in-process `InMemoryStorage` adapter; database-backed adapters (Postgres, SQLite, …) land in a later release.
