...
> **Status: available.** The outbound runtime is implemented and runs today against the in-process `InMemoryStorage` adapter; database-backed adapters (Postgres, SQLite, …) are planned.
