...
|---|---|---|---|
| **TypeScript** | Receiver: available (no release tag yet). Sender: available against the in-memory adapter; DB adapters planned. | [`postel-sh/postel`](https://github.com/postel-sh/postel) (this repo, `typescript/`) | Reference implementation. The other ports gate on passing the same compliance suite at the same version. |
