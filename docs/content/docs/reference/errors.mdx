...
**Recovery:** `401 Unauthorized`. The single most common cause is [body re-serialization](/docs/inbound/raw-bytes) — check whether your framework adapter (or a proxy in front of it) re-serializes JSON before `verify` sees it.
