...
- **Fastify's bare `verifyWebhook` preHandler** — attach the exported `releaseDedupOnError` hook as the route's `onError`. (Routes registered through `FastifyWebAdapter` install it for you.)
- **NestJS's `WebhookGuard`** — pair it with `@UseInterceptors(WebhookReleaseInterceptor)` on dedup-gated routes.
