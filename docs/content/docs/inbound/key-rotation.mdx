...
The kid is the RFC 7638 JWK thumbprint of the public key, and the sender stamps the matching `webhook-key-id` on every Ed25519 request — so receivers resolve the right key by kid. Framework adapters wrap this as a one-line `outbound.bindJwks()` mount:

```ts
// Hono — binds GET /.well-known/webhooks-keys, fetching the current keys per request
HonoWebAdapter(postel, app).outbound.bindJwks();

// Express / Fastify expose the same: ExpressWebAdapter(postel, app).outbound.bindJwks() · FastifyWebAdapter(postel, app).outbound.bindJwks()
```
