...
Each inbound source's `verify` slot accepts a single `Verifier`, a `ReadonlyArray<Verifier>`, or a named `VerifierMap` (covered [below](#naming-verifiers-instead-of-counting-on-index)). During rotation, configure the source with both new and old — producers can sign with either, and the receiver accepts both for the duration of the overlap window.
...
- **Multi-tenant receivers.** `Postel({ inbound })` sources are fixed at factory time, so per-tenant secrets assembled at request time go through the low-level `verify(rawBody, headers, secretOrKeyset)` export instead — same checks, you supply the keys per call.
...
```ts
HonoWebAdapter(postel, app).outbound.bindJwks();
```
...
```ts
ExpressWebAdapter(postel, app).outbound.bindJwks();
```
...
```ts
FastifyWebAdapter(postel, app).outbound.bindJwks();
```
...
- [Raw bytes](/docs/concepts/raw-bytes) — the silent killer of signature verification.
