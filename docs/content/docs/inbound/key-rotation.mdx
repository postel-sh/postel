...
The `Keyset(...)` strategy returns a cached, auto-refreshing keyset. The receiver looks up the public key by `kid` (extracted from the `webhook-id` header), so rotation is just *"add a new key to the JWKS, mark the old one's `not_after`, and the receiver picks it up on the next refresh."*
...
export const GET = jwksHandler({ keys: PUBLIC_JWKS });
```

`PUBLIC_JWKS` is your set of public keys in the standard JWKS shape — only public key material, never private. The handler refuses to serve a key that includes `d` (the Ed25519 private scalar). Wire it into whichever HTTP layer you use; the handler returns a `Response`.

> Note: the JWKS handler ships today, and the producer-side **signing** side (issuing keys, rotating them, recording them) is available on the [outbound](/docs/outbound) sender.
