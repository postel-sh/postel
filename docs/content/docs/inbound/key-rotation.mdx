...
> Note: the JWKS handler ships today. The producer-side **signing** side (issuing keys, rotating them automatically, recording them) lands with the [outbound](/docs/outbound) release.
...
Postel ships the consumer side of this today (the `Keyset({...})` strategy). The producer-side auto-rotation lands with the v0.2.0 sender release.
