...
> Note: the kid is the RFC 7638 JWK thumbprint of the public key, and the sender stamps the matching `webhook-key-id` on every Ed25519 request. Framework adapters will expose a one-line `.jwks()` mount that wraps this; until then, the route above works on any runtime.
