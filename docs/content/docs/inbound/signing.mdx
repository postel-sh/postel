...
2. **Timestamp window.** Reject if `|now - webhook-timestamp| > toleranceSeconds` (default 300). Throws `TimestampTooOld` — this short-circuits the entire verifier array, since timestamp validity is independent of which key signed.
