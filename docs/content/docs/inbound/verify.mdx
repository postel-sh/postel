...
`Verifier` is an open contract, not a fixed set. For any sender that isn't one of the built-ins above, anything implementing the contract works in a `verify` slot — a shared-token check, a call out to a verification service, or another provider's HMAC scheme:
