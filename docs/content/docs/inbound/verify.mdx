...
`Secret`, `PublicKey`, and `Keyset` speak Standard Webhooks. For the two highest-volume non-Standard-Webhooks senders, `@postel/core` ships ready-made verifiers instead of asking you to hand-write one:
...
```ts title="lib/postel.ts"
import { Postel, Stripe, GitHub } from "@postel/core";
import { config } from "./config.js";
...
Both are ordinary `Verifier`s — they compose in arrays/maps, report `matchedVerifierIndex`, and throw the same `SignatureInvalid`/`MalformedHeader`/`TimestampTooOld` errors as the Standard Webhooks verifiers. One important difference: the `secret` you pass to `Stripe`/`GitHub` is used as **literal UTF-8 key material**, exactly as that provider issues it (a Stripe secret keeps its `whsec_` prefix as part of the key) — it is *not* run through the `whsec_`-strip-then-base64-decode convention `Secret`/`PublicKey` use for Standard Webhooks secrets.

## Custom verifiers

`Verifier` is an open contract, not a fixed set. For any sender that isn't Standard Webhooks, Stripe, or GitHub, anything implementing the contract works in a `verify` slot — a shared-token check, a call out to a verification service, or another provider's HMAC scheme:
