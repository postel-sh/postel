...
app.post("/webhooks/vendor", express.raw({ type: "application/json" }), async (req, res) => {
  try {
    const { event } = await postel.inbound.vendor.verify(req.body, req.headers);
    res.sendStatus(200);
  } catch (err) {
    if (err instanceof SignatureInvalid)  return res.sendStatus(401);
    if (err instanceof TimestampTooOld)   return res.sendStatus(401);
    if (err instanceof MalformedHeader)   return res.sendStatus(400);
    throw err;
  }
});
```
