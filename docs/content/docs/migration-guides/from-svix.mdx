...
3. For every Svix Endpoint, call `postel.outbound.endpoints.create({ url, types: filterTypes, tenantId })`. `filterTypes` values carry over unchanged — they're the same glob semantics.
