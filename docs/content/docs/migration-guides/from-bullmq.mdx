...
| `job.attemptsMade` | `attempts.attempt_number` (via `messages.attempts(id)`) |
...
import { Postel, PgStorage, HmacV1, ExponentialBackoff, InProcess } from "@postel/core";

export const postel = Postel({
  outbound: {
    storage:     PgStorage({ connectionString: process.env.DATABASE_URL }),
    signing:     HmacV1(),
    retryPolicy: ExponentialBackoff({ maxAttempts: 5, initialDelay: "1s" }),
