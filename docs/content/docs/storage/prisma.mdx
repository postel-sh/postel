...
At runtime Postel calls Prisma only through its raw query surface (`$queryRawUnsafe` / `$executeRawUnsafe` / `$transaction`), so you add **no Postel models to your `schema.prisma`** — it works against any generated `PrismaClient`. `@prisma/client` is a peer dependency; the adapter imports only its type.
