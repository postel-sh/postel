...
- **The first-receipt branch must succeed.** If your work fails after `dedup` returns `{ duplicate: false }`, the next retry will be marked duplicate and skipped. Either move the dedup call after a successful commit, or use the host-transaction passthrough (below).
...
const adapter = inMemoryDedupAdapter({ maxEntries: 10_000 });
```
