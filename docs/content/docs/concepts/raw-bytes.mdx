...
- **Bun.serve, Deno.serve, Next.js Route Handlers, SvelteKit, Astro, Nitro** — the `Request` object already exposes the raw bytes via `await req.arrayBuffer()` before any `.json()` call. Adapters use that.
