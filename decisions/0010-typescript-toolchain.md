...
- **`@postel/compliance` is a sibling, not a `packages/` child.** Per [ADR 0006](0006-monorepo-layout.md), the compliance suite lives at the repo-root `compliance/` directory because it is a shared cross-language asset. The TS workspace pulls it in via `../compliance` in `pnpm-workspace.yaml`; the `lint` / `lint:fix` / `format` scripts pass `../compliance` to Biome alongside `.` so the sibling is covered. Trade-off: Turbo's local cache currently emits a non-fatal `paths in archives must not have ..` warning when archiving the compliance task — the build still succeeds, only the cache hit is skipped for that one package. Acceptable while compliance has zero source; revisit if it becomes a CI bottleneck (the likely fix is publishing to a remote cache, which doesn't traverse the same archive code path).
