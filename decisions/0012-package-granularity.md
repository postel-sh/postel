...
- The unified `Postel({...})` factory exposes sender and receiver through two **sub-namespaces** — `postel.outbound.*` and `postel.inbound.*` — rather than a flat method list. The sub-namespaced shape lets each side carry its own config (multi-source receivers, per-endpoint sender overrides) without polluting the other, while keeping one factory and one error hierarchy. Both sub-namespaces are independently optional via conditional types: an edge-only consumer configures just `inbound` and `postel.outbound` is not present on the instance type. See `openspec/specs/api-surface-typescript/spec.md` for the normative shape.
