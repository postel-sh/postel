...
- **Timestamp precision is milliseconds — a schema contract, not a convention.** The cursor encodes millisecond ISO-8601, so the keyset-ordered `createdAt` columns MUST store exactly millisecond precision: Postgres `timestamptz(3)` (rounds sub-ms writes to ms), MySQL `BIGINT` epoch-ms, SQLite ms ISO-8601 text. If a store holds sub-ms values (a SQL backfill, an unpinned `DEFAULT now()`, a port writing µs), rows whose fractional part the cursor truncates are silently skipped or repeated by paginated walks. Adapters do NOT truncate on read — that would defeat keyset index usage on every query; the schema enforces the precision instead, and a database created before this rule must be migrated (`ALTER TABLE … ALTER COLUMN created_at TYPE timestamptz(3)`, which rounds existing values).
- **Id tie-break requires a deterministic total order; byte order is canonical.** Two distinct ids MUST never compare equal, or the strict-inequality cursor predicate loses rows at page boundaries. Ids are opaque mixed-case tokens, so byte-order (binary collation) comparison is the canonical cross-port ordering: MySQL pins `COLLATE utf8mb4_bin` on every table (the `utf8mb4_0900_ai_ci` server default is case-insensitive and breaks the tie-break). Postgres locale collations (libc / deterministic ICU) are deterministic total orders and therefore correct — the `ORDER BY` and the cursor predicate compare through the same collation — though the visible ordering may differ cosmetically from byte order; deployments wanting byte-identical ordering across ports MAY pin `COLLATE "C"` on the id columns.
