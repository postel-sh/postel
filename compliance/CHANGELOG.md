...
> - (via OpenSpec change `compliance-real-storage-depth`) `expected.response_body_schema` (receiver mode): an embedded JSON Schema the observed response body must satisfy, in addition to and independent of the `outcome`/`error_code` verdict. Used by `jwks/public-only` to assert no JWK carries private-key material (`d`, `k`), replacing that vector's HTTP-200-only stand-in check.
> - (via OpenSpec change `compliance-real-storage-depth`) `concurrency` (receiver mode, top-level, integer ≥ 2) + `expected.outcomes` (an unordered multiset, same length as `concurrency`, replacing singular `outcome`/`error_code`): fires the same signed request N times concurrently and checks the observed verdicts as a set. Used by `receiver/dedup/concurrent-atomicity` to send a true concurrent duplicate pair instead of one request.
>
> ### CLI additions
>
...
>
> ### Storage-layer schema-conformance CI tier (via OpenSpec change `compliance-real-storage-depth`)
>
> A CI-only tier (`compliance:sender:pg:ts`, Docker-gated on `POSTEL_PG_TESTCONTAINERS`) runs the full v0.2 sender corpus against `@postel/compliance-driver` backed by real Postgres (`@postel/pg` via testcontainers) instead of only `InMemoryStorage`, then asserts the resulting `tenants`/`endpoints`/`endpoint_secrets`/`messages`/`attempts`/`endpoint_state_transitions`/`postel_received_messages` rows' column shape and enum-valued columns conform to `specs/db-schema/0001_init.sql` plus its migrations. This is a schema-conformance check riding the existing corpus, not a new per-adapter behavioral vector matrix — see "Out of scope" below for what stays deferred. Landing this surfaced and fixed a real cross-adapter bug: `messages.data` (NOT NULL in every dialect) was encoded as a bare SQL NULL for a message sent with no `data`, violating the constraint on every real SQL adapter (`@postel/storage-helpers`' `encodeMessageInsert` now uses a dedicated `encodeRequiredJson` that encodes "no data" as the JSON scalar `null` instead).
>
> ### Out of scope at v0.2.0 (deferred to v0.3+)
>
> Per the renamed *"Out-of-scope behaviors at the current MINOR"* requirement: full at-least-once crash-reclaim simulation, full per-endpoint circuit breaker state-machine assertions, full auto-disable 24-hour-window assertions, multi-tenancy fairness statistics, advanced observability, full TLS bad-cert matrix, reconciliation, replay-reconciliation chapter. Two `filtering-transformation` CONTRACTs are deferred because they are code-side host callbacks — a transform/predicate is a function, not JSON, so "Transform produces body to send" and "Filter and transform errors fail closed" cannot be expressed over the HTTP control plane until a named-callback registration mechanism is designed; they remain CONTRACT and stay covered by each port's unit suite. `storage-layer`'s deferral narrows as of `compliance-real-storage-depth`: DB-schema row/column-shape conformance for `@postel/pg` is now proven in CI (see above); the full per-adapter *behavioral* vector matrix (fanout, retry, fairness, etc. re-run once per storage adapter) remains deferred, gated by adapter packages.
