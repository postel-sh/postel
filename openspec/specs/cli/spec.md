...
The `postel` command-line binary shipped as `@postel/cli`. For v1 it is deliberately narrow: the only verb is `migrate`, which brings a target database to the current canonical schema version through the matching standalone storage adapter (`@postel/pg`, `@postel/sqlite`, `@postel/mysql`) — the operator-facing front door to the `storage-layer` capability's "Migrations runnable from CLI and programmatic API" requirement. The four other verbs once advertised in the package description (`sign`, `verify`, `replay`, `simulate`) are explicitly out of scope until a future change specs them; the binary rejects any subcommand other than `migrate`.
