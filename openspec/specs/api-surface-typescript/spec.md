...
#### Scenario: Outbound drain surface is present

- **WHEN** a TypeScript caller configures `outbound` and calls `postel.outbound.drain({ maxMessages: 100, deadline: '30s' })`
- **THEN** the call is well-typed without explicit type parameters and resolves to a `{ processed, reachedDeadline }`-shaped `DrainResult`
- **AND** `drain` does not start a persistent worker loop
