...
The current pre-alpha placeholders are `@postel/effect`, `@postel/test`, and `@postel/bun`. `@postel/nextjs` shipped a real `NextjsWebAdapter` runtime (issue #138) and `@postel/cli` shipped a real `migrate` verb (issue #148); both dropped out of this set. As each remaining placeholder's runtime lands, it drops the `__postelPackage`-only surface and may become publishable.
...
- **WHEN** a package's `src/index.ts` exports only `__postelPackage` (`@postel/effect`, `@postel/test`, `@postel/bun`)
