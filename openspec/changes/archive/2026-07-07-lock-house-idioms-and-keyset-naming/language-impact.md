...
| go-sender (planned) | unchanged | MUST mirror the locked idioms from ADR 0015 when implemented: tx in the options bag, durations accept integer seconds and the shared duration-string grammar, one clock abstraction, no colliding public names. |
...
No port exists beyond TypeScript yet, so nothing must move in lockstep. The point of landing this inside the M3 freeze is that future ports start from the locked idioms (ADR 0015) instead of inheriting the drift; they adopt the idioms at birth rather than migrating later.
