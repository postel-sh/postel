...
7. **Every requirement carries a conformance level.** Per [ADR 0008](decisions/0008-conformance-levels.md), each `### Requirement` is either NORMATIVE (default; part of the cross-port contract; will be tested by `@postel/compliance`) or IMPLEMENTATION-DEFINED (explicit opt-out; reference-implementation guidance; ports MAY vary the mechanism). To opt out, add `[IMPLEMENTATION-DEFINED]` to the requirement title AND include a `**Conformance**: IMPLEMENTATION-DEFINED` note in the body explaining what's port-specific vs what stays normative. Default is NORMATIVE — you must justify the opt-out. The compliance suite is the executable boundary between the two; what the suite tests is normative, regardless of how prose phrases it.
