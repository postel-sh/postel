...
- **Services like Svix or Hookdeck Outpost** — operational footprint (Postgres + Redis + a separate dispatcher process), $0→$490/mo cliff for hosted.
...
- Reference applications: a "minimal SaaS that sends and receives webhooks" for Next.js, Express, and Hono.
...
6. Are the receiver verifier errors actionable (which step failed and why)?
...
9. Are the wire format (AsyncAPI), DB schema (SQL), and capability specs documented well enough that the first community port (Go receiver) is plausible — and validated by passing the compliance suite?
...
- **From 1.0 onward**, all `@postel/*` packages follow strict SemVer: no breaking changes in minor or patch releases. All `@postel/*` packages share a major version and **cut each major together**; within a major, MINOR/PATCH releases ship independently per artifact (see the compliance-versioning bullet).
...
- **Compliance suite versioning shares the `@postel/*` version line.** The suite is published as a `MAJOR.MINOR` with no PATCH; every `@postel/*` package shares that line and adds its own `PATCH`. A port version `X.Y.Z` claims conformance by passing `@postel/compliance@X.Y` end-to-end before release, and the port's `PATCH` is its own bugfix space — every `X.Y.Z` conforms to the same `compliance@X.Y`. The match is on version *numbers*, not release *timing*: during `0.x` the suite is the **leading edge** — a new requirement lands in the suite first and each port adopts it on its own schedule, so the suite's latest version MAY be ahead of any port's latest release. The exception is the **MAJOR boundary** (1.0+), where the suite and all ports cut the major together (per the rule above). New tests are required at the version they ship in; breaking modifications and removals go via MAJOR. The runway-versioned evolution model sketched in [ADR 0009](decisions/0009-compliance-suite-evolution.md) is **Deferred** — it becomes operationally valuable once a second independently-maintained port lands. See [`openspec/specs/compliance/spec.md`](openspec/specs/compliance/spec.md) for the testable CONTRACT scenarios.

The combination of these distinctions — narrow CONTRACT-level surface + explicit `0.x` experimental phase + a shared version line with version-matched conformance across `@postel/*` packages — is the project's hedge against premature standardization. The architecture preserves the ability to learn operationally without locking ports into yesterday's decisions; the runway-versioned model in [ADR 0009](decisions/0009-compliance-suite-evolution.md) is the planned successor once multi-port maintainer cadences make the shared-version-line model insufficient.
